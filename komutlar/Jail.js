...
if(!data)  return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail rolünü bulamadım**`)
let data2 = await db.fetch(`jailyetkilisi_${message.guild.id}`)
if(!data2)  return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail yetkilisi rolünü bulamadım**`)
let data3 = await db.fetch(`jailkanal_${message.guild.id}`)
if(!data3)  return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail kanalını bulamadım**`)
let rol = message.guild.roles.cache.get(data)
if(!rol) return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail rolü ayarlı değil**`)
let yetkili = message.guild.roles.cache.get(data2)
if(!yetkili) return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail yetkilisi ayarlı değil**`)
let kanal = message.guild.channels.cache.get(data3)
if(!kanal) return message.channel.send(`<a:Uyar:794135977202548737>  | **Jail log kanalı ayarlı değil**`)

  if (!message.member.roles.cache.has(`${yetkili.id}`)) return message.channel.send(`<a:Uyar:794135977202548737>  | **${ayarlar.prefix}jail** isimli komutu kullanabilmek için ${yetkili} rolüne sahip olman gerekiyor.`)
  ...
  if(kişi.hasPermission("MANAGE_GUILD")) return message.channel.send(`<a:Uyar:794135977202548737>  | **Olmaz. Bu kişiyi susturamam.**`)
  
  let zaman = args[1]
  if(!args[1]) return message.channel.send(`<a:Uyar:794135977202548737>  | **Ne kadar süre jailde duracağını belirtmelisin**\nÖrnek: q!jail kişi süre sebep`)
...
db.set(`${message.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
    kanal.send(wasted)
    message.channel.send(`<:OnayPng:794135282093654047>  | ${kişi} **isimli kişi başarıyla hapishaneye gönderildi**`)
    setTimeout(async () =>{
