...
exports.run = async (client, message, args) => {
  if(message.author.id !== "748946208181059648") return message.channel.send(new Discord.MessageEmbed().setColor("RANDOM").setDescription("Way Çakal Bu Sahibimin Komutu!"))
                 if(isNaN(args[0])){
                  ...
                  if (client.guilds.cache.size < args[0]){
                   ...
                   const top = client.guilds.cache.sort((a,b)=>a.memberCount-b.memberCount).array().reverse()
                    for (let i = 0; i < args[0]; i++) {
                      ...
                      content += `${i+1}. **${top[i].name}** ~ **${top[i].memberCount}** \n`
                      }
